...
SKIP_SCHEMA_INIT="${SKIP_SCHEMA_INIT:-false}"

[[ $VERBOSE = "true" ]] && VERBOSE_MODE="--verbose" || VERBOSE_MODE=""

function initialize_hive_schema() {
  ...
  echo "Running schematool for DB_DRIVER=$DB_DRIVER with command $COMMAND ..."
  /opt/hive/bin/schematool -dbType "$DB_DRIVER" "$COMMAND" $VERBOSE_MODE
  if [ $? -eq 0 ]; then
