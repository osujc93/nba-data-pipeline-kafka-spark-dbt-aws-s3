...
"""
NBA_player_boxscores_regression.py

OVERVIEW:
---------
This script orchestrates an end-to-end pipeline for **regression** of numeric columns
(e.g. minutes_played, field_goals_made, points, plus_minus, fantasy_points, etc.)
in scd_player_boxscores using XGBoost. It closely mirrors the classification and team-based
regression pipelines but is now set up for continuous-target prediction on player-level data.
...
5) We write out all predictions (for all targets) into one Iceberg table, using
   columns like "prediction_<target>".

Changes from the team-based script:
-----------------------------------
- Now loads from "scd_player_boxscores" instead of "scd_team_boxscores".
- Replaces references of "team_id" with "player_id" for grouping (wavelet transforms).
- The target columns are now player-level numeric stats from scd_player_boxscores.
- The pipeline name and logging references updated to "NBA Player" context.
"""
...
# List of columns (in scd_player_boxscores) you want to run regression on:
...
class NBAPlayerBoxscoreRegressionPipeline:
    """
    Pipeline class that orchestrates data loading, cleaning, feature engineering,
    and regression for various numeric columns in scd_player_boxscores.
    """
    ...
    def create_spark_session(self) -> SparkSession:
        ...
        try:
            spark = (
                SparkSession.builder.appName("XGBoost_NBA_Player_Regression")
    ...
    def load_player_boxscores(self, spark: SparkSession) -> DataFrame:
        ...
        Adjust the table reference to whatever your actual location is.
        """
        start_cpu_time = time.process_time()
        start_cpu_percent = psutil.cpu_percent()
        try:
            ...
            self.logger.info("Loaded scd_player_boxscores table. Row count: %d", df_tbl.count())
            return df_tbl
        except Exception as e:
            self.logger.error("Error loading scd_player_boxscores: %s", str(e), exc_info=True)
            sys.exit(1)
        finally:
            end_cpu_time = time.process_time()
            end_cpu_percent = psutil.cpu_percent()
    ...
    def clean_data(self, df: DataFrame) -> DataFrame:
        ...
        try:
            ...
            self.logger.info("Data cleaning complete for player regression pipeline.")
            return df_dedup
        except Exception as e:
    ...
    def spark_feature_engineering(self, df: DataFrame) -> DataFrame:
        ...
        grouped by player_id (if present) and sorted by game_date (if present).
        """
        start_cpu_time = time.process_time()
        start_cpu_percent = psutil.cpu_percent()
        try:
            # Identify numeric columns (excluding player_id).
            numeric_cols = [
                f.name
                for f in df.schema.fields
                if isinstance(f.dataType, (DoubleType, IntegerType))
                and f.name != "player_id"
            ]
            ...
            # ------------------------------------------------------------------------
            # 4) Fourier/Wavelet transformations (grouped by player_id, sorted by game_date)
            # ------------------------------------------------------------------------
            ...
            if "player_id" in df.columns:
                df = df.groupBy("player_id").applyInPandas(freq_domain_features, schema=extended_schema)
            else:
    ...
    def run_lasso_feature_selection(
        ...
        df: DataFrame,
        target_column: str,
        mlflow_experiment_name: str = "NBA_Lasso_Feature_Selection_Regression_Player",
        ...
        Steps:
        1) Identify numeric columns (excluding player_id and the target_column).
        ...
        """
        # Identify numeric features (exclude the target and player_id).
        numeric_cols = [
            f.name
            for f in df.schema.fields
            if isinstance(f.dataType, (DoubleType, IntegerType))
            and f.name not in ("player_id", target_column)
        ]
        ...
        pdf = df.select(*numeric_cols, target_column).dropna().toPandas()
        if pdf.shape[0] < 20:
            self.logger.warning(
    ...
    def xgboost_training_with_hyperopt(
        ...
        df: DataFrame,
        target_column: str,
        mlflow_experiment_name: str = "XGBoost_NBA_Player_Regression",
        max_evals: int = 66
    ) -> dict[str, str]:
        """
        ...
        try:
            ...
            pdf = df.select(*feature_cols, target_column).dropna().toPandas()
            if len(pdf) < 10:
            ...
            def local_cv_score(params: dict[str, float]) -> float:
                ...
                Returns the mean RMSE across folds (Hyperopt will minimize it).
                """
                with mlflow.start_run(nested=True):
                    ...
                    for train_idx, test_idx in tscv.split(X_scaled):
                        ...
                        rmse_val = mean_squared_error(y_full[test_idx], preds, squared=False)
                    ...
                    # Hyperopt tries to minimize this
                    return mean_rmse
            ...
            with mlflow.start_run(run_name=f"XGBoost_{target_column}") as run:
                ...
                xgb_final = SparkXGBRegressor(
                    ...
                    checkpointPath="/tmp/xgb_final_checkpoints_reg_player",
                ...
                final_rmse = mean_squared_error(y_true, preds, squared=False)
                mlflow.log_metric("final_rmse", final_rmse)
                self.logger.info("[Final Model for %s] RMSE=%.4f", target_column, final_rmse)
    ...
    def generate_predictions_and_write_to_iceberg(
        ...
        try:
            ...
            # Ensure those columns exist in the DF
            ...
            out_table = f"spark_catalog.{db_name}.xgboost_player_regression_predictions_{run_ids_concat}"
    ...
    def main(self) -> None:
        """
        Main entry point for the NBA Player Boxscore Regression Pipeline.
        ...
        try:
            ...
            # 1) Load scd_player_boxscores
            df_player = self.load_player_boxscores(spark)

            # 2) Clean
            cleaned_df = self.clean_data(df_player)
            ...
            for target_col in REGRESSION_TARGET_COLS:
                ...
                selected_features = self.run_lasso_feature_selection(
                    ...
                    mlflow_experiment_name="NBA_Lasso_Feature_Selection_Regression_Player",
                ...
                model_info = self.xgboost_training_with_hyperopt(
                    ...
                    sub_df,
                    target_column=target_col,
                    mlflow_experiment_name="XGBoost_NBA_Player_Regression",
                    max_evals=66
                )
                if model_info:
            ...
            self.logger.info("Done executing main pipeline for NBA Player Regression.")
...
if __name__ == "__main__":
    pipeline = NBAPlayerBoxscoreRegressionPipeline()
    pipeline.main()
