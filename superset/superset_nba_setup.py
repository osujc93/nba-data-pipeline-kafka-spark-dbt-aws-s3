...
This script programmatically creates (or retrieves if already existing) two Trino datasets in Superset:
 ...
 - iceberg.iceberg_nba_player_boxscores.scd_team_boxscores

Then it creates:
 - Multiple charts for the 'scd_player_boxscores' dataset (player charts)
 - Multiple charts for the 'scd_team_boxscores' dataset (team charts)
...
class NBASupersetSetup:
    ...
    def get_or_create_dataset(
        ...
        if existing_id is not None:
            ...
            # Create new dataset with minimal fields
            dataset_endpoint = f"{self.superset_url}/api/v1/dataset/"
            create_payload = {
                # For creation, 'database' is still accepted as an integer
                "database": db_id,
                "schema": schema,
                "table_name": table,
            }
    ...
    def _ensure_dataset_metrics(self, dataset_id: int, metrics: List[Dict[str, Any]]) -> int:
        ...
        Returns the dataset_id (unchanged).
        """
        ...
        # IMPORTANT CHANGE: Use "database_id" instead of "database"
        ds_update_payload = {
    ...
    def create_player_charts(self, dataset_id: int) -> None:
        """
        Create various charts from the player dataset (scd_player_boxscores).
        """
        ...
        # Only charts relevant to scd_player_boxscores
    ...
    def create_team_charts(self, dataset_id: int) -> None:
        """
        Create various charts from the team dataset (scd_team_boxscores).
        """
        chart_endpoint = f"{self.superset_url}/api/v1/chart/"
        datasource_uid = f"{dataset_id}__table"
        ...
        # Charts specifically referencing scd_team_boxscores metrics
        chart_definitions: List[Dict[str, Any]] = [
            {
    ...
    def run_setup(self) -> None:
        ...
        High-level workflow to:
          ...
          3) Create or retrieve each dataset in a loop
          4) Create charts for 'scd_player_boxscores' dataset (players)
          5) Create charts for 'scd_team_boxscores' dataset (teams)
        """
        self.login()
        ...
        # Create or retrieve each dataset
