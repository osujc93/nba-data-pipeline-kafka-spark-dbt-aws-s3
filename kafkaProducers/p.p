...
# 3. Read Data
# ------------------------- #
df = spark.read.format("iceberg").load("spark_catalog.iceberg_nba_player_boxscores.scd_player_boxscores")
...
df_filtered = df.filter(col("player_id") == player_id_val)

df_filtered = df_filtered.dropDuplicates(["player_id", "game_id"])
