...
def eda_and_prepare_data(spark):
    try:
        df = spark.read.format("iceberg").load("hdfs://namenode:8020/warehouse/mlb_db/fact_boxscore")
        pdf = df.toPandas()
...
def predict_future(spark, models, features):
    try:
        ...
        future_df = spark.read.format("iceberg").load("hdfs://namenode:8020/warehouse/mlb_db/dim_future_combined_stats")
        ...
        predictions_df.write.format("iceberg").mode("overwrite").save("hdfs://namenode:8020/warehouse/mlb_db/dim_future_combined_stats")
