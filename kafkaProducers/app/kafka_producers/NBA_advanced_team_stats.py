...
PRODUCER_CONFIG: Dict[str, Union[str, int]] = {
    ...
    'linger_ms': 100,
    'batch_size': 1 * 512 * 512,   # 1 MB batch
    'max_request_size': 2 * 1024 * 1024,   # 2 MB max request size
    'compression_type': 'gzip',
    'buffer_memory': 256 * 1024 * 1024,
    'max_in_flight_requests_per_connection': 5
}

# ------------------ Pydantic Models for NBA Team Data ------------------
class NBAResultSet(BaseModel):
    name: str
    headers: List[str]
...
season_type_map: Dict[str, str] = {
    ...
    "All-Star": "All%20Star",
    "PlayIn": "PlayIn",
    "NBA Cup": "IST"  # In-Season Tournament
}
...
# Seasons from 1996-97 to 2024-25
...
FAILURES_LOG = "failures_log_advanced_team_stats.json"
...
class TokenBucketRateLimiter:
    ...
    - 'tokens_per_interval' tokens are replenished every 'interval' seconds, up to 'max_tokens'.
...
def create_topic(topic_name: str, num_partitions: int = 25, replication_factor: int = 2) -> None:
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=BOOTSTRAP_SERVERS,
            client_id='NBA_advanced_team_stats'
        )
        topic_list = [
...
def log_failure(
    ...
    """
    Append a failed combination to 'failures_log_advanced_team_stats.json' so the batch layer can re-run it.
    """
    fail_entry = {
        "season": season,
...
# ------------------ NBA Data Processing Functions ------------------
def build_nba_url(season: str, season_type: str, per_mode: str, last_n_games: int) -> str:
    """
    CHANGED: Always do Month=0 for entire season
    ADDED: &Counter=999999
    """
    base_url: str = "https://stats.nba.com/stats/leaguedashteamstats"
    return (
        f"{base_url}?Conference=&DateFrom=&DateTo=&Division=&GameScope=&GameSegment="
        f"&Height=&ISTRound=&LastNGames={last_n_games}&LeagueID=00&Location=&MeasureType=Advanced"
        ...
        f"&Counter=999999"  # <--- help avoid row-limit cutoffs
    )
...
def fetch_and_send_data(
    ...
    """
    Fetches advanced team stats data from the NBA API,
    validates it, and sends it to Kafka.
    """
    start_cpu_time: float = time.process_time()
    start_cpu_percent: float = psutil.cpu_percent()

    url: str = build_nba_url(season, season_type, per_mode, last_n_games)
    ...
    if cached_data is not None:
        data = cached_data
        logger.info(
            f"[CACHE] Using cached data for {season}, {season_type}, month=0, "
            f"per_mode={per_mode}, last_n_games={last_n_games}"
        )
    else:
        data = None
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    ...
                    logger.info(
                        f"CPU usage start: {start_cpu_percent}%, end: {end_cpu_percent}%. "
                        f"CPU time for fetch_and_send_data({season}, {season_type}, 0, {per_mode}, {last_n_games}): "
    ...
    try:
        ...
        logger.info(
            f"CPU usage start: {start_cpu_percent}%, end: {end_cpu_percent}%. "
            f"CPU time (validation error) fetch_and_send_data({season}, {season_type}, 0, {per_mode}, {last_n_games}): "
            f"{end_cpu_time - start_cpu_time:.4f} seconds"
        )
        return
        ...
        if result_set.name == "LeagueDashTeamStats":
            for row in result_set.rowSet:
                ...
                try:
                    producer.send(
                        topic,
                        key=str(row_dict.get("TEAM_ID", "0")).encode('utf-8'),
                        value=row_dict
                    )
                except (NotLeaderForPartitionError, KafkaTimeoutError) as kafka_err:
                    logger.error(
                        f"Producer error for TEAM_ID {row_dict.get('TEAM_ID', '0')}: {kafka_err}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to send message to Kafka for TEAM_ID {row_dict.get('TEAM_ID', '0')}: {e}"
                    )

    logger.info(
        f"Data for {season}, {season_type}, Month=0, {per_mode}, "
...
def process_all_combinations(producer: KafkaProducer, topic: str, max_workers: int = 1) -> None:
    """
    CHANGED max_workers to 1 for fully blocking I/O.
    CHANGED to use Month=0 only. 
    """
    start_cpu_time: float = time.process_time()
    start_cpu_percent: float = psutil.cpu_percent()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for season_str in seasons:
            for _, season_type_value in season_type_map.items():
                # We do not iterate over valid_months anymore. Single chunk: month=0
                for pm in per_modes:
                    for last_n_games in range(16):  # 0..15
                        tasks.append(
                            executor.submit(
                                fetch_and_send_data,
                                producer,
        ...
        for future in as_completed(tasks):
            try:
                ...
                logger.error(f"Error processing NBA data: {e}")

    end_cpu_time: float = time.process_time()
    end_cpu_percent: float = psutil.cpu_percent()
    logger.info(
        f"CPU usage start: {start_cpu_percent}%, end: {end_cpu_percent}%. "
        f"CPU time for process_all_combinations(): {end_cpu_time - start_cpu_time:.4f} seconds"
    )
...
def run_batch_layer(producer: KafkaProducer, topic: str) -> None:
    """
    Re-runs any combos that permanently failed in the speed layer,
    as recorded in 'failures_log_advanced_team_stats.json'.
    """
    if not os.path.exists(FAILURES_LOG):
        logger.info("No failures log found for advanced_team_stats. No batch re-runs needed.")
        return

    logger.info("Starting batch layer re-run of missing data from failures_log_advanced_team_stats.json...")
    with open(FAILURES_LOG, 'r') as f:
    ...
    logger.info("Batch layer re-run for advanced_team_stats completed.")
...
def log_stats_periodically(prof: cProfile.Profile, interval: int = 120) -> None:
    """
    Logs profiling stats every 'interval' seconds in a separate thread.
    """
    while True:
        time.sleep(interval)
        s = io.StringIO()
...
# ------------------ Main Function ------------------
def main(topic: str = "NBA_advanced_team_stats") -> None:
    """
    Speed layer + Batch layer for advanced team stats.
    """
    try:
        ...
        # 1) Speed Layer
        process_all_combinations(producer, topic, max_workers=1)

        # 2) Batch Layer re-run
        ...
        logger.info('All NBA advanced team data has been collected and sent to Kafka (speed + batch).')
