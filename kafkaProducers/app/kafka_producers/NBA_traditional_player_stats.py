...
FAILURES_LOG = "failures_log_traditional_player_stats.json"
...
def create_topic(topic_name: str, num_partitions: int = 25, replication_factor: int = 2) -> None:
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=BOOTSTRAP_SERVERS,
            client_id='NBA_traditional_player_stats'
        )
        topic_list = [
...
def build_nba_player_url(
    ...
    """
    CHANGED: Always Month=0 for entire season
    ...
    return (
        ...
        f"&GameScope=&GameSegment=&Height=&ISTRound=&LastNGames={last_n_games}&LeagueID=00&Location=&MeasureType=Base"
...
def fetch_and_send_player_data(
    ...
    if cached_data is not None:
        data = cached_data
        logger.info(
            f"[CACHE] Using cached player data for {season}, {season_type}, month=0, "
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    ...
                    logger.info(
                        f"CPU usage start: {start_cpu_percent}%, end: {end_cpu_percent}%. "
                        f"CPU time for fetch_and_send_player_data({season}, {season_type}, 0, {per_mode}, {last_n_games}): "
    ...
    try:
        ...
        logger.info(
            f"CPU usage start: {start_cpu_percent}%, end: {end_cpu_percent}%. "
            f"CPU time (validation error) fetch_and_send_player_data({season}, {season_type}, 0, {per_mode}, {last_n_games}): "
            f"{end_cpu_time - start_cpu_time:.4f} seconds"
        )
        return
    ...
    logger.info(
        f"Player data for {season}, {season_type}, Month=0, {per_mode}, LastNGames={last_n_games} sent to Kafka."
    )
...
def run_batch_layer(producer: KafkaProducer, topic: str) -> None:
    if not os.path.exists(FAILURES_LOG):
        logger.info("No failures log found for traditional_player_stats. No batch re-runs needed.")
        return

    logger.info("Starting batch layer re-run from failures_log_traditional_player_stats.json...")
    with open(FAILURES_LOG, 'r') as f:
    ...
    logger.info("Batch layer re-run for traditional_player_stats completed.")
...
# ------------------ Main Function ------------------
def main(topic: str = "NBA_traditional_player_stats") -> None:
    try:
        ...
        logger.info('All NBA player data has been collected and sent to Kafka (speed + batch).')
