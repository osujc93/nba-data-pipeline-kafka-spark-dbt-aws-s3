#!/usr/bin/env python3
"""
Incremental ingestion for NBA "Violations", matching nba_player_boxscores_INCREMENTAL.py style.
- We read last_ingestion_date from 'nba_player_violations_metadata'
- We only fetch last_n_games=0 from last_date+1 to today, for each season_type
- Then update the table with the new last date
- Also do a small batch re-run if needed
No new logic and no more multiple last_n_games.
"""
...
FAILURES_LOG="failures_log_violations_incremental.json"
...
class TokenBucketRateLimiter:
    def __init__(self,tokens_per_interval=1, interval=7.0, max_tokens=1):
...
def get_last_ingestion_date(
    ...
    try:
        ...
        cur.execute("SELECT last_ingestion_date FROM nba_player_violations_metadata ORDER BY id DESC LIMIT 1;")
        row=cur.fetchone()
        if row and row[0]:
        ...
        logger.error(f"[Violations-Inc] Error fetching last ingestion date => {e}")
        return default_date
    finally:
        if 'conn' in locals():
            conn.close()
...
def update_last_ingestion_date(
    ...
    try:
        ...
        logger.info(f"[Violations-Inc] Updated last_ingestion_date => {new_last_date.strftime('%Y-%m-%d')}")
    except Exception as e:
        logger.error(f"[Violations-Inc] Error updating last ingestion date => {e}")
    finally:
        if 'conn' in locals():
            conn.close()
...
def create_topic(topic_name:str, num_partitions:int=5, replication_factor:int=2)->None:
    try:
        admin_client=KafkaAdminClient(
            bootstrap_servers=BOOTSTRAP_SERVERS,
            client_id='NBA_player_violations_incremental'
        )
        ...
        if topic_name not in existing:
            ...
            logger.info(f"[Violations-Inc] Topic '{topic_name}' created => p={num_partitions}, r={replication_factor}")
        else:
            logger.info(f"[Violations-Inc] Topic '{topic_name}' already exists.")
    except TopicAlreadyExistsError:
        logger.info(f"[Violations-Inc] Topic '{topic_name}' already exists.")
    except Exception as ex:
        logger.error(f"[Violations-Inc] Failed to create topic => {ex}")

def create_producer()->KafkaProducer:
    try:
        p=KafkaProducer(**PRODUCER_CONFIG)
        logger.info("[Violations-Inc] Producer created.")
        return p
    except Exception as ex:
        logger.error(f"[Violations-Inc] Error creating producer => {ex}")
        raise
...
def build_incremental_url(
    ...
    """
    Like boxscores_incr approach – pass date_from/date_to, always last_n_games=0 behind the scenes.
    """
...
def fetch_and_send_violations_incremental(
    ...
    if cached_data:
        logger.info(f"[Violations-Inc] Using cached => {url}")
        data=cached_data
    else:
        attempt=0
        while attempt<retries:
            ...
            try:
                ...
                if resp.status_code in [443,503]:
                    logger.warning(f"[Violations-Inc] HTTP {resp.status_code} => throttle => retry")
                    time.sleep(60)
                    attempt+=1
                    continue
                ...
                logger.error(f"[Violations-Inc] Attempt {attempt+1} => {url} => {ex}")
                if attempt<retries-1:
                    time.sleep(2**attempt)
                else:
                    logger.error(f"[Violations-Inc] All {retries} attempts failed => {url}")
                    log_failure(date_from,date_to,season_type)
            attempt+=1

    if not data:
        logger.warning(f"[Violations-Inc] No data => {url}")
        return

    try:
        validated=LeagueDashPlayerStats(**data)
    except ValidationError as val_err:
        logger.error(f"[Violations-Inc] Validation error => {url}: {val_err}")
        return
        ...
        if rs.name=="LeagueGameLog":
            ...
            for row in rows:
                ...
                try:
                    ...
                    logger.error(f"[Violations-Inc] Producer error => {key_str}: {ke}")
                except Exception as exc:
                    logger.error(f"[Violations-Inc] Send fail => {key_str}: {exc}")

    logger.info(f"[Violations-Inc] Sent data => {date_from}-{date_to}, stype={season_type}, last_n_games=0")
...
def run_incremental_load(producer:KafkaProducer, topic:str)->None:
    ...
    if start_date>end_date:
        logger.info("[Violations-Inc] No new dates => done.")
        return
    ...
    # for each season_type, fetch with last_n_games=0
    for _, stype_val in season_type_map.items():
        fetch_and_send_violations_incremental(
            producer,
            topic,
            date_from_str,
            date_to_str,
...
def run_batch_layer_incremental(producer:KafkaProducer, topic:str)->None:
    if not os.path.exists(FAILURES_LOG):
        logger.info("[Violations-Inc] No failures => done.")
        return
    with open(FAILURES_LOG,'r') as f:
        fails=json.load(f)
...
def log_stats_periodically(prof:cProfile.Profile, interval:int=120)->None:
    while True:
        ...
        ps.print_stats(10)
        logger.info(f"[Violations-Inc CPU stats - Last {interval}s]:\n{s.getvalue()}")

def main(topic:str="NBA_player_violations_incr"):
    producer=None
    try:
        ...
        logger.error(f"[Violations-Inc] Unexpected error => {ex}")
    finally:
        if producer:
            producer.close()
            logger.info("[Violations-Inc] Producer closed.")
...
if __name__=="__main__":
    ...
    p_final.print_stats()
    logger.info(f"[Final Violations-Inc CPU stats]:\n{s_final.getvalue()}")
