...
PRODUCER_CONFIG: Dict[str, Union[str, int]] = {
    ...
    'max_request_size': 2 * 1024 * 1024,   # 2 MB max
...
FAILURES_LOG = "failures_log_traditional_team_stats.json"
...
def create_topic(topic_name: str, num_partitions: int = 25, replication_factor: int = 2) -> None:
    ...
    try:
        admin_client = KafkaAdminClient(
            bootstrap_servers=BOOTSTRAP_SERVERS,
            client_id='NBA_traditional_team_stats'
        )
        topic_list = [
...
# ------------------ Batch Layer Helper ------------------
def log_failure(season: str, season_type: str, per_mode: str, last_n_games: int) -> None:
    fail_entry = {
        "season": season,
...
def build_nba_url(season: str, season_type: str, per_mode: str, last_n_games: int) -> str:
    """
    CHANGED: Month=0 for entire season
    ...
    return (
        ...
        f"&Height=&ISTRound=&LastNGames={last_n_games}&LeagueID=00&Location=&MeasureType=Base"
...
def fetch_and_send_data(
    ...
    if cached_data is not None:
        data = cached_data
        logger.info(
            f"[CACHE] Using cached data for {season}, {season_type}, month=0, {per_mode}, last_n_games={last_n_games}"
        )
    else:
        data = None
        ...
        if result_set.name == "LeagueDashTeamStats":
            for row in result_set.rowSet:
                ...
                try:
                    ...
                    logger.error(f"Producer error for TEAM_ID {row_dict.get('TEAM_ID', '0')}: {kafka_err}")
                except Exception as e:
                    logger.error(f"Failed to send message to Kafka for TEAM_ID {row_dict.get('TEAM_ID', '0')}: {e}")

    logger.info(
        f"Data for {season}, {season_type}, Month=0, {per_mode}, LastNGames={last_n_games} sent to Kafka."
    )
...
def process_all_combinations(producer: KafkaProducer, topic: str, max_workers: int = 1) -> None:
    ...
    CHANGED to skip month iteration and do single chunk (Month=0).
    """
    start_cpu_time: float = time.process_time()
    start_cpu_percent: float = psutil.cpu_percent()
...
def run_batch_layer(producer: KafkaProducer, topic: str) -> None:
    if not os.path.exists(FAILURES_LOG):
        logger.info("No failures log found for traditional_team_stats. No batch re-runs needed.")
        return

    logger.info("Starting batch layer re-run from failures_log_traditional_team_stats.json...")
    with open(FAILURES_LOG, 'r') as f:
    ...
    logger.info("Batch layer re-run for traditional_team_stats completed.")
...
# ------------------ Main Function ------------------
def main(topic: str = "NBA_traditional_team_stats") -> None:
    try:
        ...
        logger.info('All NBA traditional team data has been collected and sent to Kafka (speed + batch).')
