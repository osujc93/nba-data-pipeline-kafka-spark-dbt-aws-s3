...
per_modes: List[str] = [
    "Totals", "PerGame", "Per100Possessions", "Per100Plays", "Per48",
...
def create_producer() -> KafkaProducer:
    ...
    try:
        ...
        logger.info("Kafka Producer created successfully (Misc).")
        return producer
    except Exception as e:
...
def build_nba_team_url(
    ...
    """
    Build the leaguedashteamstats URL with MeasureType=Misc.
    ADDED &Counter=999999 as well
    """
...
def fetch_and_send_team_data(
    ...
    if cached_data is not None:
        data = cached_data
        logger.info(
            f"[CACHE] Using cached misc data for "
    ...
    logger.info(
        f"[SUCCESS] Misc data for {season}, {season_type}, Month={month}, {per_mode}, LastNGames={last_n_games} sent to Kafka."
    )
...
def process_all_misc_combinations(producer: KafkaProducer, topic: str, max_workers: int = 1):
    """
    CHANGED: max_workers=1 => blocking, and chunk approach => month=0 only.
    """
    start_cpu_time = time.process_time()
    start_cpu_percent = psutil.cpu_percent()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for season_str in seasons:
            for _, season_type_value in season_type_map.items():
                chunked_months = [0]  # entire season only
    ...
    logger.info(
        f"CPU usage start: {start_cpu_percent}%, end: {end_cpu_percent}%. "
        f"CPU time for process_all_misc_combinations: {end_cpu_time - start_cpu_time:.4f} seconds"
    )

# ------------------ BATCH LAYER ------------------
def run_batch_layer(producer: KafkaProducer, topic: str):
    if not os.path.exists(FAILURES_LOG):
        logger.info("No failures log found. No batch re-run for misc needed.")
        return

    logger.info("Starting batch layer re-run for 'NBA_misc_team_stats' from failures log...")
    with open(FAILURES_LOG, 'r') as f:
    ...
    logger.info("Batch re-run completed for 'NBA_misc_team_stats'.")
...
# ------------------ MAIN ------------------
def main(topic: str = "NBA_misc_team_stats"):
    try:
        ...
        logger.info("All misc data has been collected and sent to Kafka (speed + batch).")
        ...
        try:
            producer.close()
            logger.info("Kafka Producer closed (Misc).")
        except Exception as e:
