# file: traditional_teams_spark_nlp.py
...
def create_spark_connection() -> SparkSession:
    try:
        spark: SparkSession = (
            SparkSession.builder.appName("Traditional_Teams_SparkNLP")
            .enableHiveSupport()
            .getOrCreate()
        )
        logger.info("Spark connection created for traditional teams Spark NLP.")
        return spark
...
def load_traditional_team_stats(spark: SparkSession) -> DataFrame:
    try:
        ...
        logger.info("Loaded traditional team stats from Iceberg.")
        return df
    except Exception as e:
...
def store_embeddings_in_qdrant(df: DataFrame, collection_name: str) -> None:
    try:
        ...
        logger.info(f"Stored {len(points)} traditional team embeddings into Qdrant.")
    except Exception as e:
...
def main() -> None:
    ...
    store_embeddings_in_qdrant(result_df, "traditional_teams_embeddings")

    spark.stop()
    logger.info("Traditional teams Spark NLP script completed successfully.")
